...
/**
 * Upgrade code for the formulas question type.
 * @param int $oldversion the version we are upgrading from.
 * @return bool true if upgrading works.
 */
function xmldb_qtype_formulas_upgrade($oldversion = 0) {
    ...
    if ($oldversion < 2012071407) {
        ...
        $questions = $DB->get_records('question', ['qtype' => 'formulas'], 'id');
        foreach ($questions as $question) {
            $anscount = 0;
            $rs = $DB->get_recordset('qtype_formulas_answers', ['questionid' => $question->id], 'id');
