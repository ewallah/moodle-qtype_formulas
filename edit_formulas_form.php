...
/**
 * Ccoodinate question type editing form definition.
 *
 * @package   qtype_formulas
 * @copyright 2013 Jean-Michel Vedrine
 * @author    Hon Wai, Lau <lau65536@gmail.com>
...
class qtype_formulas_edit_form extends question_edit_form {
    ...
    protected function definition_inner($mform) {
        ...
        $mform->addElement('text', 'globalunitpenalty',
                get_string('globaloptions', 'qtype_formulas') . get_string('unitpenalty', 'qtype_formulas'), ['size' => 3]);
    ...
    protected function get_per_answer_fields($mform, $label, $gradeoptions, &$repeatedoptions, &$answersoption) {
        ...
        // Part's mark.
        $repeated[] = $mform->createElement('text', 'answermark', get_string('answermark', 'qtype_formulas'), ['size' => 3]);
        ...
        $repeated[] = $mform->createElement('hidden', 'numbox', '', '');
        // Exact value will be computed during validation.
        ...
        // Part's answer.
        $repeated[] = $mform->createElement('text', 'answer', get_string('answer', 'qtype_formulas'), ['size' => 80]);
        ...
        $repeated[] = $mform->createElement('text', 'correctness', get_string('correctness', 'qtype_formulas'), ['size' => 60]);
        ...
        // Part's unit penalty.
        $repeated[] = $mform->createElement('text', 'unitpenalty', get_string('unitpenalty', 'qtype_formulas'), ['size' => 3]);
        $repeatedoptions['unitpenalty']['default'] = $config->defaultunitpenalty;
        $repeatedoptions['unitpenalty']['helpbutton'] = ['unitpenalty', 'qtype_formulas'];
        ...
        // Part's placeholder.
        $repeated[] = $mform->createElement('text', 'placeholder', get_string('placeholder', 'qtype_formulas'), ['size' => 20]);
        ...
        $repeated[] = $mform->createElement('editor', 'partpartiallycorrectfb',
            get_string('partiallycorrectfeedback', 'qtype_formulas'), ['rows' => 3], $this->editoroptions);
     ...
     * @param object $gradeoptions the possible grades for each answer.
     * @param array $minoptions the minimum number of answer blanks to display.
     * @param array $addoptions the number of answer blanks to add.
     */
                ...
                foreach ($question->options->answers as $key => $answer) {
                    ...
                    foreach ($fields as $field) {
                        ...
                        $defaultvalues[$field . '[' . $key . ']'] =
                            ['text' => $fieldtxt, 'format' => $answer->$fieldformat, 'itemid' => $itemid];
                    }
                    ...
                    foreach ($fields as $field) {
                        ...
                        $itemid = file_get_submitted_draft_itemid($field . '[' . $key . ']');
                        $fieldtxt = file_prepare_draft_area($itemid, $this->context->id, 'qtype_formulas',
                            $field, empty($answer->id) ? null : (int)$answer->id, $this->fileoptions, $answer->$field);
                        $defaultvalues[$field . '[' . $key . ']'] =
                            ['text' => $fieldtxt, 'format' => $answer->$fieldformat, 'itemid' => $itemid];
                    }
                }
